...
class Calculator(QWidget):
    def __init__(self):
        ...
        for button in buttons:
            ...
            print(button_obj.height())
            fontsize = button_obj.height() // 20
            button_obj.setStyleSheet("PushButton { font-size: %dpx; text-align: center;"
                                     "background-color: rgb(250, 250, 250); border-radius: 5px;}" % fontsize)  # 设置按钮文本的字体大小和居中对齐
            col += 1
            if col > 3:
                col = 0
                row += 1
    ...
    def setButtonStyle(self):
        ...
        # 设置按钮文本的样式表
        button_style = "PushButton { font-size: %dpx; text-align: center; }" % font_size
        print(font_size)
